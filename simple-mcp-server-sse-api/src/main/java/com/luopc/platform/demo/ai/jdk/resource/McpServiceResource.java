...
public class McpServiceResource {
    ...
    @Tool(name = "getJdkLink", description = "get download link via jdk version")
    public String getJdkLink(@McpToolParam(description = "jdk version", required = true) int jdkVersion) {
        log.info("JDK Version: {}", jdkVersion);
        String link = javaVersionService.getDownloadUrl(JavaVersion.getJavaVersion(jdkVersion));
